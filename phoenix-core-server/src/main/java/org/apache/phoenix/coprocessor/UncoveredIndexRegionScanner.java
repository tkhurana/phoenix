...
public abstract class UncoveredIndexRegionScanner extends BaseRegionScanner {
  ...
  public UncoveredIndexRegionScanner(final RegionScanner innerScanner, final Region region,
  ...
  protected Scan prepareDataTableScan(Collection<byte[]> dataRowKeys,
    ...
    if (!keys.isEmpty()) {
      ...
      dataScan.setAttribute(SERVER_PAGE_SIZE_MS, Bytes.toBytes(Long.valueOf(pageSizeMs)));
      return dataScan;
    } else {
    ...
    do {
      ...
      if (!row.isEmpty()) {
        ...
        lastIndexRowKey = ImmutableBytesPtr.copyBytesIfNecessary(firstCell.getRowArray(),
          firstCell.getRowOffset() + offset, firstCell.getRowLength() - offset);
  ...
  private boolean verifyIndexRowAndRepairIfNecessary(Result dataRow, byte[] indexRowKey,
    long indexTimestamp) throws IOException {
  ...
  protected boolean getNextCoveredIndexRow(List<Cell> result) throws IOException {
    if (indexRowIterator.hasNext()) {
      ...
      try {
        ...
        Result dataRow = dataRows.get(new ImmutableBytesPtr(indexToDataRowKeyMap.get(indexRowKey)));
        ...
        if (dataRow != null) {
          ...
          if (
            ...
            if (tupleProjector != null) {
              IndexUtil.addTupleAsOneCell(result, new ResultTuple(dataRow), tupleProjector, ptr);
            }
          } else {
            result.clear();
          }
          ...
          if (indexMaintainer.isUncovered()) {
            long ts = indexRow.get(0).getTimestamp();
            // Since we also scan the empty column for uncovered global indexes, this mean the data
            // row
   ...
   * from the data table regions in parallel. These data rows are then joined with index rows. The
   * join is for adding uncovered columns to index rows. This implementation conforms to server
   * paging such that if the server side operation takes more than pageSizeInMs, a dummy result is
   * returned to signal the client that more work to do on the server side. This is done to prevent
   * RPC timeouts.
