 ...
 * clusters for nulls(empty result). If we get an empty result from one cluster but are unable get a
 * result from the other cluster we either return a null that we got or throw an error based on
 * {@link #ERROR_ON_SINGLE_NULL_ATTRIB} This gives some additional consistency for this specific
 * case at the cost of some latency and behavior
 */
public class ParallelPhoenixNullComparingResultSet extends DelegateResultSet
  ...
  public ParallelPhoenixNullComparingResultSet(ParallelPhoenixContext context,
    ...
    this.errorOnSingleNull = Boolean.valueOf(context.getProperties()
      .getProperty(ERROR_ON_SINGLE_NULL_ATTRIB, DEFAULT_ERROR_ON_SINGLE_NULL));
  }
  ...
  public boolean next() throws SQLException {
    ...
    if (this.rs == null) {
      ...
      CompletableFuture<Boolean> candidate1 = ParallelPhoenixUtil.INSTANCE
        .getFutureAndChainOnContext(function, rs1, context::chainOnConn1,
      ...
      CompletableFuture<Boolean> candidate2 = ParallelPhoenixUtil.INSTANCE
        .getFutureAndChainOnContext(function, rs2, context::chainOnConn2,
      ...
      try {
        if (notEmpty) {
          ...
          Pair<CandidateResult<Boolean>, CandidateResult<Boolean>> candidateResultPair =
            findFirstNonExceptionallyCompletedCandidateResult(candidateResult1, candidateResult2);
          ...
          try {
            secondResult = ParallelPhoenixUtil.INSTANCE
              .getFutureNoRetry(candidateResultPair.getSecond().getCandidate(), context);
          } catch (Exception e) {
            LOG.warn(
            ...
            if (errorOnSingleNull) {
              ...
              throw new SQLExceptionInfo.Builder(
                SQLExceptionCode.HA_READ_FROM_CLUSTER_FAILED_ON_NULL).setRootCause(e)
